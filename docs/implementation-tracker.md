...
| Admin 관리 화면 (M16) | 구현 완료(기기 관리 E2E 미검증) | 스케줄/예약, 시스템 기본정보, 도면, 지역(Area) 관리는 E2E 검증 완료. 기기 등록/수정/연결 설정/소프트 폐기 UI·API 구현 완료, 정적 검증 및 repository 테스트 통과 |
...
상태: 구현 완료 (5개 영역 모두 구현. **기기 등록/설정은 실인프라·브라우저 E2E 미검증**, 나머지 4개
영역은 E2E 검증 완료, 2026-07-10 신설 — 사용자 요청: 기기 등록/설정, 지역 관리, 도면 관리, 시스템 기본정보
관리, 스케줄/예약 등록 화면 추가)
...
- ADMIN이 위 5개 영역을 UI에서 등록/수정할 수 있다(기기/지역/도면/Site·Building 기본정보/스케줄).
  **5개 관리 영역 구현 완료. 기기 등록/설정만 실인프라·브라우저 E2E 검증이 남음.**
...
5. **M16 기기 관리 E2E 마감** — 기기 생성(`buildDeviceBase()` 토픽), 기본정보 수정, 연결 프로토콜 설정,
   `DECOMMISSIONED` 소프트 폐기를 실인프라와 브라우저에서 검증. 구현과 정적 검증은 완료됨
   (자세한 내용은 [M16](#m16-admin-관리-화면-공간기기도면스케줄-관리-콘솔) 참고)
