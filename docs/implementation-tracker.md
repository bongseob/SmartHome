...
- 작성일: 2026-07-09 (최근 갱신: 2026-07-14 — M12/M13 실기기·MQTT 인증 진행 반영)
...
| 운영 보안 | 진행 중 | Mosquitto auth/ACL(보드별 계정 발급 + ACL, `allow_anonymous` 폐지) 완료(2026-07-13). TLS(mqtts/wss), 서비스간 mTLS/개별 API Key는 미구현(현재 서비스는 공용 계정 `svc-backend` 사용) |
...
상태: 진행 중 (Mosquitto auth/ACL 완료, 2026-07-13. TLS·서비스간 mTLS는 미완료)

작업:
- Mosquitto TLS 설정 — 미완료(여전히 평문 1883/9001)
- WSS/mqtts only — 미완료
- auth/ACL plugin 결정 및 연동 — **완료(2026-07-13)**: `allow_anonymous false` +
  password_file/acl_file, 보드별 계정 발급 스크립트(아래 완료 단위 참고)
- service API key 또는 mTLS — 미완료(현재는 서비스 전용 공용 계정 `svc-backend` 1개로 대체, 서비스별
  분리는 아님)
- `.env.example` 정리 — 완료(신규 인증 흐름 반영)

완료 조건:
- dev-only 평문 포트와 production TLS 구성이 명확히 분리된다. 미완료
...
알려진 단순화(후속 필요):
- TLS(mqtts/wss)는 여전히 평문 — production 배포 전 필수
- 서비스 간은 여전히 공용 계정 1개(`svc-backend`) — mTLS/개별 API Key로 분리할지는 A.2 "배포/인프라"
  결정과 함께 논의 필요
...
전환됐다(상세는 각 마일스톤 섹션·§4 참고).

다음은 두 갈래 후보가 있다:
1. **M13 나머지(TLS·서비스간 mTLS)** — 지난 auth/ACL 작업의 연장선. production 배포 전 반드시 필요하고,
   범위가 명확해 사용자 결정 없이 바로 착수 가능하다.
2. **M11 AI/HITL** — SRS 3.5의 마지막 미구현 핵심 도메인 기능. 다만 confidence threshold·고위험 장치
   목록 등 정책값은 구현 전 사용자 확인 필요(부록 A.2와 별개로, 이 마일스톤 고유의 열린 값들).

둘 중 어느 쪽도 급하지 않다면(운영 배포 시점이 아직 아니라면) M11을 먼저 추천한다 — SRS 핵심 기능
공백을 먼저 메우는 편이 명령/audit 불변식 원칙(아래)에 더 부합한다.
...
1. M11 AI/HITL — 추천 저장, confidence threshold, 고위험 기기 게이트, approve/reject, 학습데이터 저장
   (정책값은 구현 전 사용자 확인 필요)
2. M13 나머지 — Mosquitto TLS(mqtts/wss), 서비스간 mTLS 또는 계정 개별 분리
