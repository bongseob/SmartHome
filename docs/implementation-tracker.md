...
| Admin 관리 화면 (M16) | 부분 완료 | 스케줄/예약, 시스템 기본정보(Site/Building 이름), 도면(Floor Map) 관리 완료(모두 E2E 검증됨). 기기 등록/설정, 지역(Area) 관리는 미완료(백엔드 CRUD API부터 필요) |
...
상태: 부분 완료 (5개 영역 중 **스케줄/예약·시스템 기본정보·도면 관리 완료, 기기 등록/설정·지역(Area)
관리 미완료**, 2026-07-10 신설 — 사용자 요청: 기기 등록/설정, 지역 관리, 도면 관리, 시스템 기본정보
관리, 스케줄/예약 등록 화면 추가)
...
1. **기기 등록/설정** (SRS 2.1.2)
   ...
   - 백엔드 신규 필요: Device 생성/삭제 API(현재는 connection PATCH만 있고 CRUD 자체가 없음 — seed만
     INSERT 가능)
2. **지역(Area) 관리** (SRS 2.1.1)
   - Area 생성/수정/삭제, Polygon 편집(Floor Map 캔버스 위에 드로잉 — M8 편집 모드의 좌표계 재사용)
   - 백엔드 신규 필요: Area CRUD API(현재 `listAreasByFloor`만 read-only로 존재)
...
- ADMIN이 위 5개 영역을 UI에서 등록/수정할 수 있다(기기/지역/도면/Site·Building 기본정보/스케줄).
  **스케줄·시스템 기본정보·도면 완료. 기기 등록/설정·지역(Area) 관리는 미완료.**
...
4. `apps/api/src/routes/spatial.controller.ts`: `POST floors/:id/floor-map`(multipart, `FileInterceptor`
   + `diskStorage`, 파일명은 `randomUUID()` + 원본 확장자, png/jpg/jpeg/webp/svg만 허용, 10MB 제한),
...
5. **M16 Admin 관리 화면** — 기기 등록/설정, 지역(Area) 관리, 도면(Floor Map) 관리, 시스템 기본정보
   (Site/Building 이름) 관리, 스케줄/예약 등록 화면. 5개 영역 중 스케줄러만 백엔드 API 완비, 나머지는
   백엔드 CRUD부터 신규 필요(자세한 내용은 [M16](#m16-admin-관리-화면-공간기기도면스케줄-관리-콘솔) 참고)
