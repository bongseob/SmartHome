...
전환됐고, 2026-07-14에 M13의 TLS **설정 준비**(mqtts/wss, 자체 서명 CA, ESP32 TLS 스위치)까지
마쳤다(상세는 각 마일스톤 섹션·§4 참고).

M13은 이제 "설정은 끝났지만 실제 배포 인프라에서 검증되지 않은" 상태다 — 배포 인프라(K8s/VM 등,
부록 A.2)가 정해지기 전까지는 더 진행해도 검증할 방법이 없다. 남은 후보는 두 갈래:
1. **M11 AI/HITL** — SRS 3.5의 마지막 미구현 핵심 도메인 기능. confidence threshold·고위험 장치
   목록 등 정책값은 구현 전 사용자 확인 필요(부록 A.2와 별개로, 이 마일스톤 고유의 열린 값들).
2. **배포 인프라 결정(부록 A.2)** — 이걸 먼저 정하면 M13(TLS 실배포 검증)과 M15(운영/성능)를
   동시에 구체화할 수 있다. 다만 이는 구현 작업이 아니라 사용자 결정이 선행돼야 하는 항목이다.

M11을 먼저 추천한다 — SRS 핵심 기능 공백을 먼저 메우는 편이 명령/audit 불변식 원칙(아래)에 더
부합하고, 배포 인프라와 무관하게 지금 바로 착수할 수 있다.
