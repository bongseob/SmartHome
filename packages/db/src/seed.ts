...
/**
 * 개발용 시드 (idempotent). device-simulator M1 의 thermostat-01 과 정합.
...
async function seed(): Promise<void> {
  ...
  const floorId = await idOf(
    `INSERT INTO floor (building_id, slug, name) VALUES ($1, '2f', '2F')
     ON CONFLICT (building_id, slug) DO UPDATE SET name = EXCLUDED.name RETURNING id`,
    [bldgId],
  );
  const areaId = await idOf(
    `INSERT INTO area (floor_id, slug, name) VALUES ($1, 'living-room', '거실')
     ON CONFLICT (floor_id, slug) DO UPDATE SET name = EXCLUDED.name RETURNING id`,
    [floorId],
  );

  const mqttTopic = buildDeviceBase({
    site: "site1",
    building: "bldg-a",
    floor: "2f",
    area: "living-room",
  ...
  await query(
    `INSERT INTO device (code, name, category, device_type, mqtt_topic, area_id, current_status)
     VALUES ('thermostat-01', '거실 온도조절기', 'SENSOR', 'thermostat', $1, $2, 'OFFLINE')
     ON CONFLICT (code) DO UPDATE SET mqtt_topic = EXCLUDED.mqtt_topic, area_id = EXCLUDED.area_id`,
    [mqttTopic, areaId],
  );
  ...
  console.log(`[seed] 완료 — area=${areaId}, device thermostat-01 (topic=${mqttTopic}), admin=admin`);
}
